...
public class NuevoBarrioCtl extends GenericForwardComposer implements Serializable{
	...
	public void onClick$guardar(){	
		...
		BarrioServicio 	barrioServicio = (BarrioServicio)SpringUtil.getBean("barrioServicio");
		try {
			barrioServicio.guardar(
					descripcion.getText(),
					usuarioConectado,
			...
			Pintor pintor=(Pintor)Sessions.getCurrent().getAttribute("pintor");
			if(pintor!=null ){	
				pintor.abrirVentana("logistico/web/vista/ListaBarrios.zul","contenidoPosicion",null);
			}
		} catch (Exception e) {
