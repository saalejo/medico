...
public class NuevoModuloSistemaCtl extends GenericForwardComposer implements Serializable{
	...
	public void onClick$guardar(){	
		...
		ModuloSistemaServicio 	moduloSistemaServicio = (ModuloSistemaServicio)SpringUtil.getBean("moduloSistemaServicio");
		try {
			moduloSistemaServicio.guardar(
					descripcion.getText(),
					usuarioConectado
			...
			Pintor pintor=(Pintor)Sessions.getCurrent().getAttribute("pintor");
			if(pintor!=null ){	
				pintor.abrirVentana("logistico/web/vista/ListaModuloSistema.zul","contenidoPosicion",null);
			}
		} catch (Exception e) {
